...
public class GoTController {
    ...
    public void exportarXML(ActionEvent actionEvent) {
        if (!nombre_doc.getText().isEmpty()){
            ...
            try {
                ...
                // Exportar a archivo
                File file = new File("res/" + nombre_doc.getText() + ".xml");
    ...
    public void exportarJSON(ActionEvent actionEvent) {
        if (!nombre_doc.getText().isEmpty()){
            ...
            try {
                objectMapper.writerWithDefaultPrettyPrinter().writeValue(new File("res/" + nombre_doc.getText() + ".json"), SearchHolder.getInstance().getCharacterItems());
            } catch (IOException e) {
                e.printStackTrace();
            }
            vboxOk.setVisible(true);
    ...
    public void exportarBIN(ActionEvent actionEvent) throws IOException {
        if (!nombre_doc.getText().isEmpty()){
            doc_vacio.setVisible(false);
            ObjectOutputStream objectStream = new ObjectOutputStream(new FileOutputStream("res/" + nombre_doc.getText() + ".bin"));
            try {
